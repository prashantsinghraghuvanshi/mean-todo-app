...
@Component({
  ...
  template: `
    <div class="w-full max-w-2xl">
      <h2 class="text-2xl font-bold mb-4 text-blue-400">In Progress Tasks</h2>
      ...
      <ul *ngIf="todos.length" class="space-y-3">
        <li *ngFor="let todo of todos" class="bg-gray-800 rounded-lg px-4 py-3">
            ...
            <div class="flex items-start gap-3 flex-1">
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  <span [class.line-through]="todo.completed" class="text-lg font-semibold">{{ todo.title }}</span>
                  <span class="text-xs bg-blue-600 text-white px-2 py-1 rounded">In Progress</span>
                </div>
                <p *ngIf="todo.description" class="text-sm text-gray-400 mt-1">{{ todo.description }}</p>
                <p *ngIf="todo.due_date" class="text-sm text-yellow-400 mt-1">📅 Due: {{ todo.due_date }}</p>
              </div>
            </div>
          </div>
...
export class InProgressTodosComponent {
  @Input() todos: any[] = [];
